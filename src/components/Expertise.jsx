    ...
    >
      ...
      <div className="grid md:grid-cols-3 gap-6">
        {expertiseData.map((item, index) => (
          <div
            key={index}
            className="expertise-card opacity-0 translate-y-10 transition-all duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] rounded-2xl border border-[#c1c6d7]/10 bg-white/80 backdrop-blur-sm p-8 hover:shadow-xl hover:-translate-y-1 transition-all duration-400 group"
            style={{ transitionDelay: `${index * 100}ms` }}
          >
            <div className="mb-5 text-[#1b1b1e] group-hover:scale-110 transition-transform duration-300">
              {item.icon}
            </div>
            <h3 className="font-['Playfair_Display'] text-3xl font-normal text-[#1b1b1e] mb-3">
              {item.title}
            </h3>
